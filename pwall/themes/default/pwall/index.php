<?php
/*-------------------------------------
// PaymentWall addon Created by kenik
// This addon allows you to get
// donations for your server through
// PaymentWall system:
//     https://www.paymentwall.com/
---------------------------------------
// Contact:
// Email: kenik2006@gmail.com
// Hercules: http://herc.ws/board/user/9024-kenik/
// Skype: kenik2006
-------------------------------------*/
if (!defined('FLUX_ROOT')) exit; ?>
<p>Your current balance: <?=$session->account->balance?> credits</p>
<?php
$attr = array(
    'frameborder' => '0',
    'width' => Flux::config('PWallWidgetWidth'),
    'height' => Flux::config('PWallWidgetHeight')
);
echo $widget->getHtmlCode($attr); ?>